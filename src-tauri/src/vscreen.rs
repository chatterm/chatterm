...
impl Perform for ScreenInner {
    ...
    fn csi_dispatch(&mut self, params: &Params, _intermediates: &[u8], _ignore: bool, action: char) {
        ...
        match action {
            'A' => { self.cursor_row = self.cursor_row.saturating_sub(p1); } // cursor up
            'B' => { self.cursor_row = (self.cursor_row + p1).min(self.rows - 1); } // cursor down
            'C' => { self.cursor_col = (self.cursor_col + p1).min(self.cols - 1); } // cursor right
            'D' => { self.cursor_col = self.cursor_col.saturating_sub(p1); } // cursor left
            'H' | 'f' => { // cursor position
            ...
            'J' => { // erase display
                ...
                match mode {
                    0 => { // erase below
                        ...
                        for r in (self.cursor_row + 1)..self.rows { self.clear_line(r); }
                    }
                    1 => { // erase above
                        for r in 0..self.cursor_row { self.clear_line(r); }
                    }
                    2 | 3 => { // erase all
                        for r in 0..self.rows { self.clear_line(r); }
                    }
                    _ => {}
                }
            }
            'K' => { // erase line
                ...
                if row < self.rows {
                    match mode {
                        0 => { for c in self.cursor_col..self.cols { self.grid[row][c] = ' '; } }
                        1 => { for c in 0..=self.cursor_col.min(self.cols - 1) { self.grid[row][c] = ' '; } }
                        2 => { self.clear_line(row); }
                        _ => {}
                    }
                }
            }
            'G' => { self.cursor_col = (p1.saturating_sub(1)).min(self.cols - 1); } // cursor horizontal absolute
            'd' => { self.cursor_row = (p1.saturating_sub(1)).min(self.rows - 1); } // cursor vertical absolute
...
impl VScreen {
    ...
    pub fn extract_last_message(&self, agent: Option<&str>) -> Option<String> {
        ...
        for row in rows.iter().rev() {
            ...
            if cleaned.is_empty() || cleaned.len() < 2 { continue; }

            let result = if cleaned.chars().count() > 60 {
                let end = cleaned.char_indices().nth(57).map(|(i, _)| i).unwrap_or(cleaned.len());
                format!("{}…", &cleaned[..end])
            } else {
                cleaned.to_string()
            };
    ...
    pub fn detect_state(&self, agent: Option<&str>) -> Option<String> {
        ...
        for row in rows.iter().rev() {
            ...
            if agent == Some("claude") {
                ...
                if t.starts_with('❯') { return Some("idle".to_string()); }
            }

            // Kiro: status bar with "Kiro · auto" = idle, spinner ● with animation = thinking
            if agent == Some("kiro") {
                if tl.starts_with("kiro") && tl.contains("auto") { return Some("idle".to_string()); }
                // Kiro shows ● with animated text when thinking
                if t.starts_with('●') && (tl.contains("...") || tl.contains("…")) { return Some("thinking".to_string()); }
            }

            // Codex: > prompt visible = idle, "working" text = thinking
            if agent == Some("codex") {
                if tl.contains("working") { return Some("thinking".to_string()); }
                // Codex status bar with model info = idle
                if tl.contains("gpt-") && tl.contains("xhigh") { return Some("idle".to_string()); }
            }
        }
        None
    }
...
fn is_chrome(line: &str, agent: Option<&str>) -> bool {
    ...
    if agent == Some("claude") || t.contains("claude") {
        if t.contains("context") && t.contains("usage") && t.contains("%") { return true; }
        if t.contains("mcp") && len < 20 { return true; }
        if t.contains("opus") && t.contains("context") { return true; }
        ...
        if t.contains("no recent activity") { return true; }
        if t.contains("recent activity") && len < 30 { return true; }
        if t.contains("imageinclipboard") || t.contains("ctrl+v") { return true; }
        if t.contains("/effort") || t.contains("xhigh") || t.contains("xlow") { return true; }
        // Trust prompt
        if t.contains("yes, i trust") || t.contains("no, exit") { return true; }
        if t.contains("accessing workspace") { return true; }
        if t.contains("quick safety check") { return true; }
        if t.contains("be able to read, edit") { return true; }
        if t.contains("security guide") { return true; }
        // User input (❯ prefix) — filter ALL ❯ lines, they're either empty prompt or user typing
        if line.trim_start().starts_with('❯') { return true; }
        // Thinking spinners (✢ Proofing…, ✳ Moonwalking…, etc.)
        if line.trim_start().starts_with('✢') || line.trim_start().starts_with('✳')
            || line.trim_start().starts_with('✶') || line.trim_start().starts_with('✻')
            || line.trim_start().starts_with('✽') { return true; }
        // Organization/path info
        if t.contains("organization") && t.contains("@") { return true; }
    }

    // === Kiro CLI chrome ===
    if agent == Some("kiro") || t.contains("kiro") {
        if t.contains("/copy to clipboard") { return true; }
        if t.contains("ask a question or describe a task") { return true; }
        if t.starts_with("kiro") && t.contains("auto") { return true; }
        if t.contains("credits:") && t.contains("time:") { return true; }
        if t.contains("mcp failure") { return true; }
        if t.contains("welcome to the new kiro") { return true; }
        if t.contains("thinking") && t.contains("esc to cancel") { return true; } // thinking indicator
        if t.contains("prefer the classic experience") { return true; }
        if t.contains("/tui to learn more") { return true; }
    }

    // === Codex chrome ===
    if agent == Some("codex") || t.contains("codex") || t.contains("openai") {
        if t.contains(">_ openai codex") { return true; } // banner
        if t.contains("model:") && t.contains("/model") { return true; }
        if t.contains("directory:") { return true; }
        if t.contains("/skills to list") { return true; }
        if t.contains("gpt-") && t.contains("xhigh") { return true; } // model status
        if t.starts_with("tip:") { return true; }
        if t.starts_with(">") && len < 5 { return true; } // empty prompt
    }

    // === Common chrome ===
    if t.contains("security guide") { return true; }
    if t.contains("enter to confirm") { return true; }
    if t.contains("initializing") && len < 30 { return true; }
    ...
    if line.trim_start().starts_with('>') && !line.trim_start().starts_with(">>") && len < 80 { return true; }
...
mod tests {
    ...
    fn test_claude_chrome_detection() {
        assert!(is_chrome("Context ████ 3% │ Usage ████ 2% (1h 57m / 5h) │ ████ 11% (3d 18h / 7d)", Some("claude")));
        ...
        assert!(!is_chrome("● 你好。需要我帮你看代码、跑命令，还是处理别的事情？", Some("claude")));
    }

    #[test]
    fn test_kiro_chrome_detection() {
        assert!(is_chrome("/copy to clipboard", Some("kiro")));
        assert!(is_chrome("ask a question or describe a task ↵", Some("kiro")));
    ...
    fn test_codex_chrome_detection() {
        ...
        assert!(is_chrome("model:     gpt-5.4 xhigh   /model to change", Some("codex")));
        assert!(is_chrome("gpt-5.4 xhigh · ~/my_project/chat_term_tests", Some("codex")));
        assert!(is_chrome("Tip: New Use /fast to enable our fastest inference", Some("codex")));
